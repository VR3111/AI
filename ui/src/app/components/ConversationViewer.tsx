...
export function ConversationViewer({ conversation, onClose, onSubmitQuery, isProcessing = false }: ConversationViewerProps) {
  ...
  return (
    <div className="h-full min-w-0 flex flex-col bg-background">
      <div className="px-3.5 py-4 sm:p-4 lg:p-6 bg-card/50 backdrop-blur-sm border-b border-border/50 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h2 className="mb-1 text-lg lg:text-xl">Conversation History</h2>
        ...
        <button
          onClick={onClose}
          className="w-full sm:w-auto sm:self-start px-3.5 py-2.5 lg:px-5 lg:py-2.5 bg-secondary/50 hover:bg-secondary text-secondary-foreground rounded-lg lg:rounded-xl transition-all duration-150 lg:duration-200 border border-border/30 hover:border-border/60 relative overflow-hidden group text-sm text-center shrink-0"
        >
      ...
      <div className="flex-1 overflow-y-auto px-3.5 py-4 sm:p-4 lg:p-8 pb-32 lg:pb-8">
        <div className="max-w-3xl mx-auto w-full space-y-6 lg:space-y-8">
          {conversation.turns.map((turn, idx) => (
            <div key={idx} className="space-y-3 lg:space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300 lg:duration-500" style={{ animationDelay: `${idx * 30}ms` }}>
              {/* User's Question */}
              <div>
                <div className="text-xs text-muted-foreground mb-2 lg:mb-3 uppercase tracking-wider">
                  Your Question
                </div>
                <div 
                  className="p-3 lg:p-5 bg-card/50 backdrop-blur-sm border border-border/50 rounded-lg lg:rounded-xl shadow-sm relative group"
                ...
                >
                  ...
                  <div className="flex items-start gap-2 lg:gap-3 relative">
                    ...
                    <div className="flex-1 text-sm lg:text-base text-foreground/90 leading-relaxed">{turn.query}</div>
      ...
      <div className="fixed lg:relative bottom-0 left-0 right-0 px-3.5 py-4 sm:p-4 lg:p-6 bg-gradient-to-t from-card via-card/95 to-card/80 backdrop-blur-xl border-t border-border/50">
