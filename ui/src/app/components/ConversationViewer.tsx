...
export function ConversationViewer({ conversation, onClose, onSubmitQuery, isProcessing = false }: ConversationViewerProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      <div className="p-4 lg:p-6 bg-card/50 backdrop-blur-sm border-b border-border/50 flex items-center justify-between">
        <div>
        ...
        <button
          onClick={onClose}
          className="px-3 py-2 lg:px-5 lg:py-2.5 bg-secondary/50 hover:bg-secondary text-secondary-foreground rounded-lg lg:rounded-xl transition-all duration-150 lg:duration-200 border border-border/30 hover:border-border/60 relative overflow-hidden group text-sm"
        >
          ...
          {conversation.turns.map((turn, idx) => (
            <div key={idx} className="space-y-3 lg:space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300 lg:duration-500" style={{ animationDelay: `${idx * 30}ms` }}>
              ...
              <div>
                ...
                <ResponseRenderer response={turn.response} />
              </div>
            </div>
          ))}
        </div>
